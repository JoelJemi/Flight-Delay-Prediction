...
def load_data():
    global df,data,x,y, x_train, y_train, acc_rf, acc_dt, acc_br, acc_gbr, acc_svm
    if request.method == "POST":
...
def prediction1():  
    if request.method=="POST":
        ...
        model = joblib.load('random_forest_arr.pkl')
...
def prediction2():  
    if request.method=="POST":
        ...
        model = joblib.load('random_forest_dept.pkl')
        result=model.predict([lee])
        print(result)
        if result<=0:
